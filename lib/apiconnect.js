(function($) {
  ...
  function getProtocol(setting, params) {
    ...
    if(setting == 'auto') {
      protocol = window.location.protocol == 'https:' || tokenExists(params) ? 'https' : 'http';
    }
  ...
  function switchToJSONP(context, options) {
    return ((!options.cors || !$.support.cors) && options.jsonp && options.format == 'json' && !isSameDomain(context));
  }

  function isSameDomain(context) {
    return window.location.hostname == context.domain() && window.location.port == context.port();
  }
