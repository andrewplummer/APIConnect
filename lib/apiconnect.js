...
(function(context) {
  ...
  var DEFAULT_OPTIONS = ['protocol', 'domain', 'port', 'timeout', 'cors', 'jsonp'];
  ...
  function connectRoute(str, routeOptions) {
    ...
    if(context[as]) {
      ...
      context[as] = function(params, options) {
        var path, key, callback, deferred;
        ...
        path = resolvePath(context, routeObject, params, options);

        deferred = initiateRequest(context, path, method, params, options);
  ...
  function resolvePath(context, routeObject, params, options) {
    var path = '', options;
    ...
    arrayEach(routeObject, function(fragment) {
      ...
      if((paramsExpected && found) || (!paramsExpected && !fragment.param)) {
        path += '/' + (fragment.param ? params[found] : fragment.text);
      }
    ...
    if(options.appendFormat) {
      path += '.' + (typeof options.appendFormat == 'string' ? options.appendFormat : options.format);
    }
    return path;
  }

  function initiateRequest(context, path, method, params, options) {
    var deferred, cached, protocol, fullURL, split;

    protocol = getProtocol(options.protocol, params);
    ...
    fullURL = util.getFullURL(util.getURLBase(protocol, options) + path, params);
    ...
    if(util.splitRequest(fullURL, options)) {
      if(options.sizeError) {
        ...
        deferred = [];
        split = options.sizeError.call(context, path, params);
        ...
        if(util.isArray(split)) {
          ...
          arrayEach(split, function(request) {
            deferred.push(initiateRequest(context, request.path, method, request.params, options));
          });
    ...
    deferred = util.ajax(protocol, method, path, params, options);
  ...
  function switchToJSONP(context, options) {
    return ((!options.cors || !util.supportsCORS()) && options.jsonp && options.format == 'json' && !isSameDomain(context));
  }
  ...
  // that can latter be overwritten in node.
  ...
  var util = {
    ...
    ajax: function(protocol, method, path, params, options) {
      var base = util.getURLBase(protocol, options);
      options.data = params;
      options.type = method;
      return $.ajax(base + path, options);
    },
    when: function() {
    ...
    getURLBase: function(protocol, options) {
      var base = protocol;
      base += '://';
      base += options.domain;
      if(options.port) {
