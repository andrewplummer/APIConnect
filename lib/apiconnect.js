...
(function(context) {
   ...
   * @extra Domain can also be set when creating a new instance of APIConnect by passing a string as the first argument to the constructor.
   *
   * @example
   *
   ...
   * @extra When active, APIConnect will attempt to make cross-browser ajax requests IF they are supported by the browser. JSONP will be used instead of standard ajax if the following conditions are met: 1. This setting is false OR 2. Th browser does not support CORS AND 3. The API is not the same domain. Note that CORS needs to be enabled on the server-side as well. If the API you're trying to connect to does not support this feature it should be turned off.
   *
   * @example
   *
   *   api.cors(false) -> api
   ...
   * @extra Default parameters are added to the end of all routes used. This is useful for things like API keys or access tokens that need to be used in every API call.
   * @example
   *
   ...
   * @extra Default options are merged into the local options of all routes when called, and apply to all connected routes.
   * @example
   *
   ...
   * @extra Some APIs require a POST method to stand in for PUT and DELETE. This option turn all PUT and DELETE requests into POST with a %_method% parameter representing the true method. <setting> can be %true% or %false%.
   * @example
   *
   ...
   * @returns Nothing
   * @short Allows a callback inside which <context> will be prepended.
   ...
   *
   ...
   +   api.resource('tweets', { only: 'get,create' })   -> Connects get and create routes
   +   api.resource('tweets', { except: 'get,create' }) -> Connects index, update, and delete routes
