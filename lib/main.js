(function($) {
  ...
  function getMethodName(method, routeObject, routingOptions) {
    ...
    name += camelize(obj.name || obj.text);
    return name;
  }
  ...
  function initiateRequest(context, url, method, params, options) {
      ...
      if(options.sizeError) {
        ...
        if($.isArray(split)) {
          ...
          return $.when.apply(deferred).fail(function(data) {
            callOptionalCallback(context, options.error, data);
          }).done(function(data) {
            callOptionalCallback(context, options.complete, data);
            callOptionalCallback(context, options.success, data);
          });
    ...
    cacheCapture(context, deferred, fullURL, options);
  ...
  function cacheCapture(context, deferred, fullURL, options) {
    if(options.cache) {
  ...
  InstanceMethods['authorize'] = function(url, params) {
    ...
    openExternalURL(context, url, popup, openerFields);
  ...
  function openExternalURL(context, url, popup, openerFields) {
    if(popup) {
      ...
      interval = setInterval(function() {
        getAccessToken(context, child, function(token) {
          if(token) {
      ...
      $('<iframe style="display:none;" src="'+ url +'"></iframe').appendTo(document.body).load(function() {
        var frame = this;
        getAccessToken(context, this.contentWindow, function(token) {
          $(frame).remove();
          if(!token) {
  ...
  function getAccessToken(context, win, fn) {
    var match;
    try {
      match = win.location.hash.match(/(access_token)=(\w+)/);
    } catch(e) {}
    if(match) {
      context.param('access_token', match[2]);
      fn(match[2]);
    } else {
      fn();
    }
