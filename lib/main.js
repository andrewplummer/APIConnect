(function() {
  ...
  */
  ...
  function getMethodName(method, routeObject, routingOptions) {
    var name = HTTP_ACTIONS[method].human, reversed;
  ...
  var HTTP_ACTIONS = {
    'GET':    { name: 'get',  human: 'get',     collection_has_id: true  },
    'POST':   { name: 'post', human: 'create',  collection_has_id: false },
    'PUT':    { name: 'put',  human: 'update',  collection_has_id: true  },
    'DELETE': { name: 'del',  human: 'destroy', collection_has_id: true  }
  };
  ...
  iterateOverObject(HTTP_ACTIONS, function(method, obj) {
  ...
  InstanceMethods['resource'] = function(name, opt) {
    opt = opt || {};
    var context = this,
        singular = singularize(name),
        isPlural = singular != name,
        isCollection = opt.collection || isPlural;

    iterateOverObject(HTTP_ACTIONS, function(method, obj) {
      var route = name;
      if(isCollection && obj.collection_has_id) {
        route += '/:id';
      }
      context[obj.name](route, { name: singular, collection: opt.collection });
    });

    // If it's a collection then add an index route.
    if(isCollection && isPlural) {
      context.get(name);
    }
  }
