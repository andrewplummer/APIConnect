(function($) {
  ...
  var DEFAULT_OPTIONS = ['protocol', 'domain', 'port', 'format', 'timeout', 'cors', 'jsonp'];
  ...
  function connectRoute(context, method, route, routingOptions) {
    ...
    if(context[methodName]) {
      ...
      context[methodName] = function(params, localOptions) {
        ...
        deferred = initiateRequest(url, method, params, options);
  ...
  function mergeDefaults(context, obj) {
    ...
    arrayEach(obj.routes, function(r) {
      var match = r.toLowerCase().match(/\s*(get|post|put|delete)\s+\/?(\S+)\s*(?:as\s*(.+))?/i),
          opt = {},
          method, route, as;
      if(match) {
        method = match[1];
        route  = match[2];
        opt.as = match[3];
        context[method].call(context, route, opt);
      }
    });
  }
  ...
  InstanceMethods['key'] = getterOrSetter('defaultParams', 'api_key');
